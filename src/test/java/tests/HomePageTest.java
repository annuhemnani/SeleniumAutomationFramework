...
public class HomePageTest extends BaseTest {
	 ...
	 * sendKeys("Testing Mini bytes - Youtube",Keys.ENTER); String
	 * title=DriverManager.getDriver().getTitle(); System.out.println(title);
	 * Assertions.assertThat(title) .isNotNull() .isNotBlank()
	 * .containsIgnoringCase("google search") .matches("\\w.*"+"google search")
	 ...
	 * 
	 * List<WebElement>
	 * elements=DriverManager.getDriver().findElements(By.xpath("//h3/span"));
	 * //assertEquals(elements.size(), 10); boolean isElementPresent=false;
